<?php
	...
	echo $dateStart;
	echo date_format($dateStart, 'd/m/Y');
	echo date_format($start, 'Y-m-d');*/
	$datoStart = explode("/", $start); 

	$dateStart = $datoStart[2].'/'.$datoStart[1].'/'.$datoStart[0]; 
	$fechaStart = date_create($dateStart);
	$fechaInicio = date_format($fechaStart, 'Y/m/d');

	$datoEnd = explode("/", $end); 

	$dateEnd = $datoEnd[2].'/'.$datoEnd[1].'/'.$datoEnd[0]; 
	$fechaEnd = date_create($dateEnd);
	$fechaFin = date_format($fechaEnd, 'Y/m/d');


	if($fechaStart>$fechaEnd){
		echo json_encode(['error' => true, 'message' => 'Error en el ingreso de las fechas, la fecha final debe estar posterior a la fecha incial']);
		return;
	}
    ...
    if ($_SESSION['type'] == "Admin") {
		...
		$incidence = @mysql_query("SELECT i.idIncidencia, p.name, c.name, i.severidad, i.created_at, i.state, i.resumen
						...
						WHERE i.created_at >= '".$fechaInicio."' AND i.created_at <= '".$fechaFin."'
		...
		$incidence = @mysql_query("SELECT i.idIncidencia, p.name, c.name, i.severidad, i.created_at, i.state, i.resumen
						...
						WHERE ia.idUsuario = '".$_SESSION['idUsuario']."' AND ia.enable=1 AND i.created_at >= '".$fechaInicio."' AND i.created_at <= '".$fechaFin."'")or die ('Error al seleccionar la Base de Datos: '.@mysql_error());
		...
		$incidencias = $incidencias.'<tr class="'.$row[5].'">
										...
										<td class="center">
											<div class="hidden-sm hidden-xs btn-group">
												...
												<a href="verSeguimiento.php?idIncidencia='.$row[0].'" class="btn btn-xs btn-info">
	...
	echo json_encode(['error' => false, 'message' => $incidencias]);
	return;
?>